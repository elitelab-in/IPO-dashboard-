...
def get_search_suggestions():
    ...
    from services.metadata_service import _service
    results = []
...
def calculate_sector_analysis_sync():
    from services.sector_calc_layer import SectorCalcLayer
...
def get_fpi_nsdl_api():
    try:
        from services.fpi_nsdl_service import FpiNsdlService
...
def author_dashboard():
    return send_from_directory(app.template_folder, 'author.html')
...
def create_article():
    ...
    try:
        cursor.execute('''
            INSERT INTO articles (title, slug, content, category, read_time_minutes, image_url, author_id)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (title, slug, content, category, read_time, image_url, session['user_id']))
        conn.commit()
        conn.close()
        return jsonify({'message': 'Article created successfully', 'slug': slug}), 201
    except Exception as e:
        conn.close()
        return jsonify({'error': str(e)}), 500
...
def get_articles():
    ...
    cursor = conn.cursor()
    cursor.execute('''
        SELECT a.id, a.title, a.slug, a.category, a.read_time_minutes, a.image_url, a.created_at, u.name as author_name
        FROM articles a
